...
const CardContainer = memo(
  forwardRef<HTMLDivElement, CardContainerProps>(({ children }, ref) => (
    <div ref={ref} className="py-4 flex items-center justify-center opacity-0">
      {children}
    </div>
  )),
)
...
const Card = memo(({ data }: CardProps) => {
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      gsap.to(card, { rotationX: 0, rotationY: 0, scale: 1, duration: 0.6, ease: 'elastic.out(1, 0.3)' })
    }
  ...
  return (
    <CardContainer ref={containerRef}>
      <div
        ref={cardRef}
        className="w-full max-w-[260px] md:max-w-[300px] lg:max-w-[320px] h-[250px] md:h-[270px] lg:h-[280px] rounded-xl flex flex-col items-center text-center 
        overflow-hidden bg-gray-100/5 backdrop-blur-md border border-transparent
        transition-transform duration-300 ease-in-out hover:border-white/20
        transform-gpu perspective-1000"
      >
        <div className="relative w-32 md:w-36 h-32 md:h-36 mt-4 group overflow-hidden">
          <Image
            src={data.imageSrc}
            alt="Card Image"
            fill
        ...
        <p className="text-gray-200 text-xs md:text-sm mt-4 px-3 md:px-4">{data.description}</p>
      </div>
    </CardContainer>
  )
})
...
export default function Dashboard() {
  return (
    <div className="min-h-screen bg-[#00040F] text-white px-4 md:px-6 py-6 md:py-8">
      <main className="rounded-2xl p-4 md:p-40 lg:p-60">
